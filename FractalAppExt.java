...
public class FractalAppExt {
    ...
    static final class FractalGenerator {
        private static volatile boolean slowMode = true; // for demo/testing
                ...
                double angleDeg, double scale)
        {
            ...
            if (slowMode) { try {Thread.sleep(100);} catch (InterruptedException ie) { Thread.currentThread().interrupt(); return; } }
            ...
            handle.queue.offer(new Line(x, y, x2, y2, argb));
            ...
            if (!handle.isCancelled.get()) {
                int asyncX = x2;
                int asyncY = y2;
                double asyncAngle = angle + angleDeg;
                submitAsync(handle, () -> submitBranch(handle, asyncX, asyncY, asyncAngle, nextLen, nextHeight, angleDeg, scale));
            }
            submitBranch(handle, x2, y2, angle - angleDeg, nextLen, nextHeight, angleDeg, scale);
        }
        ...
        static void generateSet(FractalControl handle, FractalType type, int w, int h, int maxIter,
            ...
            for (int y = 0; y < h && !handle.isCancelled.get(); y++) {
                final int rowIndex = y;
                submitAsync(handle, () -> computeRow(handle, type, w, h, maxIter, xmin, ymin, scaleX, scaleY, juliaRe, juliaIm, rowIndex));
            }
        }

        private static void computeRow(
                FractalControl handle,
                FractalType type,
                int w, int h,
                int maxIter,
            ...
            int[] row = new int[w];
            double yRatio = (h <= 1) ? 0.0 : (rowIndex / (double)(h - 1));
            double imag = ymin + yRatio * scaleY;
            double denomW = (w <= 1) ? 1.0 : (w - 1);
            for (int x = 0; x < w; x++) {
                if (handle.isCancelled.get()) {
                    return;
                }
                double xRatio = (w <= 1) ? 0.0 : (x / denomW);
                double real = xmin + xRatio * scaleX;
                row[x] = switch (type) {
            ...
            if (!handle.isCancelled.get()) {
                handle.queue.offer(new RowInfo(rowIndex, row));
            }
        }

        private static void submitAsync(FractalControl handle, Runnable task) {
            if (handle.isCancelled.get()) {
                return;
            }
        ...
        private static void completeTask(FractalControl handle) {
